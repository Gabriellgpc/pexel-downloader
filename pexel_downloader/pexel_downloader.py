...
class PexelDownloader:
    ...
    def download_images(self, query, num_images, save_directory, size='original', page=1):
        ...
        def process_image(img):
            ...
            # Log details to CSV
            self._log_to_csv(content_id=img['id'],
                            author_name=author,
                            profile_url=profile_url,
                            file_path=img_name,
                            content_type="image",
                            csv_file=os.path.join(save_directory, "downloaded_content.csv"))
        Parallel(n_jobs=-1)(delayed(process_image)(img) for img in tqdm(images, desc="Downloading ..."))

    def download_videos(self, query, num_videos, save_directory, size='medium', page=1):
        if not os.path.exists(save_directory):
            os.makedirs(save_directory)
        ...
        def process_video(video):
            ...
            # Log details to CSV
            self._log_to_csv(content_id=video['id'],
                            author_name=author,
                            profile_url=profile_url,
