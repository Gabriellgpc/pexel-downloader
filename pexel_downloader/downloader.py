...
@click.option("--start-page", "-p", default=1, type=int, help="Page number to start downloading from.")
def main(query, num, content_type, size, save_directory, start_page):
    """
    ...
    \b
    \tpexel-downloader nature 5 video --start-page 2
    """
    ...
    # Passed start_page to both download_images and download_videos functions
    if content_type.lower() == "image":
        downloader.download_images(query=query, num_images=num, save_directory=save_directory, size=size, page=start_page)
    else:
        downloader.download_videos(query=query, num_videos=num, save_directory=save_directory, size=size, page=start_page)
